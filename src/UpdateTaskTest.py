...
class TestClass:
    ...
    def test_normal_result(self, initialization):
        ...
        taskCommand = UpdateTask(
            jenisTask="tubes", namaMatkul="IF2230", deadlineBaru="2021-05-02")
        taskCommand.execute()
        ...
        assert "Yay deadline tugas telah diperbaharui!!!" in result
        print("--- RESULT ---")
        print(result)
        print("--- END OF RESULT ---")
    ...
    def test_with_no_result(self, initialization):
        taskCommand = UpdateTask(
            jenisTask="tucil", namaMatkul="IF2230", deadlineBaru="2021-05-03")
        taskCommand.execute()
        result = taskCommand.getResult()
    ...
    def test_task_not_found(self, initialization):
        taskCommand = UpdateTask(
            jenisTask="tubes", namaMatkul="IF2220", deadlineBaru="2021-05-03")
        taskCommand.execute()
        result = taskCommand.getResult()
